 ...
 * 9. Re-reconciled at merge time (2026-07-08, after Agents 0/1B/1C/2/3/4/5/6
 *    landed on main): all 17 fields below still exist live with the expected
 *    api_name/data_type — no drift affecting Agent 1A's own logic. One
 *    additive change found: Outreach_Status now carries an 8th live value,
 *    `Converted` (not created by Agent 1A — likely Agent 2/Onboarding marking
 *    a prospect who became an ambassador). Agent 1A's code doesn't enumerate
 *    or validate against a fixed picklist value set, so this is not a
 *    breaking change, just noted here for anyone auditing the shared field.
 */
