...
/**
 * Zoho CRM helper for Agent 2 — metadata + records.
 *
 * Reuses the metadata-helper pattern from functions/agent5A/zoho.js (token
 * refresh + crmGet + fetchModules/fetchFields), extended the same way
 * functions/agent0/zoho.js was: module/field resolution + record CRUD, plus a
 * compound-criteria search the daily compliance sweep (Function C) needs.
 *
 ...
 * `verifyFields` cross-checks the field names Agent 2 writes against Zoho's
...
/**
 * Compound AND search. `conditions` is [{ field, operator, value }] — operator
 * is a Zoho search operator (equals, starts_with, ...). Builds
 * `((f1:op:v1)and(f2:op:v2))`. Used by the Function C daily compliance sweep.
 */
async function searchByConditions(moduleApiName, conditions) {
  const criteria = conditions.map((c) => `(${c.field}:${c.operator}:${c.value})`).join('and');
  return searchByCriteria(moduleApiName, conditions.length > 1 ? `(${criteria})` : criteria);
}
...
async function getRecord(moduleApiName, id) {
  const res = await crmGet(`/${encodeURIComponent(moduleApiName)}/${encodeURIComponent(id)}`);
  if (res.status === 204) return null;
  if (res.status !== 200 || !res.body) {
    throw new Error(`get failed on "${moduleApiName}/${id}" (HTTP ${res.status}): ${JSON.stringify(res.body).slice(0, 200)}`);
  }
...
/**
 * Pull all records from a module matching a compound condition (paged). Used by
 * the Function C daily sweep to find compliance-incomplete ambassadors.
 */
async function fetchAllByConditions(moduleApiName, conditions, { fields, perPage = 200, maxPages = 100 } = {}) {
  const criteria = conditions.map((c) => `(${c.field}:${c.operator}:${c.value})`).join('and');
  const criteriaStr = conditions.length > 1 ? `(${criteria})` : criteria;
  const fieldParam = fields && fields.length ? `&fields=${encodeURIComponent(fields.join(','))}` : '';
  const out = [];
  let page = 1;
  while (page <= maxPages) {
    const res = await crmGet(
      `/${encodeURIComponent(moduleApiName)}/search?criteria=${encodeURIComponent(criteriaStr)}&page=${page}&per_page=${perPage}${fieldParam}`,
    );
